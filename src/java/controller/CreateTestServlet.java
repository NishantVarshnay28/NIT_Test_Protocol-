...
public class CreateTestServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String t2 = request.getParameter("duration");
            t2=t2.concat(":00");
            Time duration= Time.valueOf(t2);
            ...
            if(faculty!=null){
            testCode= td.addTestDetails(batch, branch, subCode, date, time, duration,faculty.gettId());
            }
            else{
                System.out.println("Faculty is not in session ");
            }
            session.setAttribute("testCode", testCode);
            ArrayList<Question> qlist=new ArrayList();
            session.setAttribute("qlist", qlist);
            RequestDispatcher rd=request.getRequestDispatcher("/facultyViews/testPage2.jsp");
